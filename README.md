...
- 🧩 **适配新版 Antigravity IDE**：面向新版 Antigravity / Antigravity 2.0 使用场景整理丰富中文词表。
- 💬 **侧边栏对话标题智能汉化**：针对 AI 动态生成的英文对话标题（如 "Basic Chinese Greeting"），内置技术短语翻译引擎，实现侧边栏对话名实时转为中文。
- ⚙️ **AI 全局规则自动配置**：安装时自动向全局配置（`~/.gemini/config/GEMINI.md`）注入中文命名与回复规则，全方位提升汉化体验。
- 🤖 **覆盖 Agent 核心界面**：重点汉化 Agent 管理、任务状态、模型选择、设置弹窗、常用操作按钮等界面文案。
- 🛡️ **安全非侵入**：不破解、不修改 Antigravity 客户端二进制本体文件，降低更新损坏风险。
- ⚡ **轻量且智能**：基于 Node.js 驱动，配合系统原生守护机制（macOS LaunchAgent / Windows 计划任务），开机自启，后台静默运行。
- 🔄 **实时界面同步**：动态监听客户端 DOM 与窗口加载，全面覆盖核心 UI 菜单与文案。
...
> [!WARNING]
> - **代码与正文保护**：为避免影响开发与代码运行，代码块（Code Block）、终端内容（Terminal）以及用户自定义消息内容不会被强制翻译，以保留原始交互与上下文。
